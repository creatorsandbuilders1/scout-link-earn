/**
 * Supabase Edge Function: decline-project
 * 
 * Allows a Talent to decline a project proposal.
 * This validates the project state before the frontend calls 
 * the decline-project function on the project-escrow-v2 smart contract.
 * 
 * Expected payload:
 * {
 *   projectId: number,
...
serve(async (req) => {
    ...
    try {
        ...
        return new Response(
            JSON.stringify({
                success: true,
                message: 'Project verified, ready for decline',
                project: {
