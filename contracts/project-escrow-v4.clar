;; project-escrow-v4.clar
;; The heart of REFERYDO! - Version 4 with Manual Underflow Protection
;; Manages project creation, escrow, talent acceptance/decline, and atomic payouts.

;; =====================================================
;; CHANGELOG v4:
;; - CRITICAL FIX: Replaced checked-sub (non-existent) with manual underflow check
;; - Uses asserts! to verify total-fees <= total-amount before subtraction
;; - Native Clarity functions only - guaranteed to deploy successfully
;; =====================================================

;; =====================================================
;; CHANGELOG v3:
;; - FAILED DEPLOYMENT: checked-sub is not a native Clarity function
;; - Attempted to use checked-sub for safe arithmetic (does not exist)
;; =====================================================
...
(define-constant ERR-FEE-CALCULATION-ERROR (err u105))  ;; v4: For arithmetic errors
...
;; ============================================================================
;; APPROVE AND DISTRIBUTE FUNCTION - v4 CRITICAL FIX
;; ============================================================================
;; Approves the project and distributes the funds. Must be called by the Client.
;; v4 FIX: Uses manual underflow check with native Clarity functions only
;; ============================================================================
(define-public (approve-and-distribute (project-id uint))
  (let ((project (unwrap! (map-get? projects project-id) ERR-PROJECT-NOT-FOUND)))
    ...
    (let
      ...
      ;; --- v4 CRITICAL FIX: Manual Underflow Check ---
      ;; Verify that total-fees does not exceed total-amount before subtraction
