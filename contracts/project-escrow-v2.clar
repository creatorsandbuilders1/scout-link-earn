;; project-escrow-v2.clar
;; The heart of REFERYDO! - Version 2 with Proposal & Acceptance Flow
...
;; MODIFIED v2: Now sets status to u4 (Pending_Acceptance) instead of u1 (Funded)
(define-public (fund-escrow (project-id uint))
  (let ((project (unwrap! (map-get? projects project-id) ERR-PROJECT-NOT-FOUND)))
    ...
    ;; Update status to "Pending_Acceptance" (v2 change)
    (map-set projects project-id (merge project {status: u4}))
    (ok true)
  )
)

;; NEW v2: Allows the Talent to accept the project.
...
;; NEW v2: Allows the Talent to decline the project with automatic refund.
...
;; NO CHANGES in v2 - works the same as v1
